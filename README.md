...
- 광주, 담양, 여수, 순천, 목포 지역별 50개 로컬 장소 데이터
- DB 없이 정적 HTML/CSS/JavaScript로 실행
- API 장애 상황에서도 발표 가능한 로컬 우선 구조
- 로컬 실행 시 TourAPI 실시간 장소 데이터 추가 연동
- 사진이 포함된 장소 추천 카드
- 날씨별 최종 일정
...
GitHub Pages 배포판은 API 키 없이 로컬 장소 데이터로 실행됩니다.
로컬 실행판은 `config.local.js`의 키를 내 컴퓨터 안에서만 읽어 TourAPI 실시간 장소 데이터와 `detailCommon2`의 장소 상세 설명을 추가로 불러옵니다.
...
주의: GitHub Pages처럼 정적 사이트에 직접 넣은 키는 브라우저에서 보일 수 있습니다. 그래서 TourAPI와 남도 API 키는 `config.local.js`에만 두고, 로컬 서버(`local-server.js`)가 대신 호출합니다. Kakao Maps JavaScript 키는 도메인 제한을 걸어 사용하는 방식이 일반적입니다.
...
실시간 TourAPI까지 쓰려면 로컬 서버로 실행합니다.

Windows에서는 아래 파일을 실행합니다.
...
로컬 실행에서는 화면이 자동으로 `/api/places`를 호출합니다. 프록시가 TourAPI 목록과 상세 설명을 가져오고, 상세 설명 호출이 실패한 장소는 기존 로컬 설명으로 자동 대체됩니다. API 호출이 실패해도 지역별 50개 로컬 장소 데이터로 계속 작동합니다.
